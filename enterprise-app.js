...
const SERVICE_SCHEMAS = Object.freeze({
  'income-certificate': {
    id: 'income-certificate', prefix: 'INC', service: 'Income Certificate', icon: '▣', estimatedMinutes: 3, totalFields: 18,
  ...
  'scholarship-application': {
    id: 'scholarship-application', prefix: 'SCH', service: 'Scholarship Application', icon: '☆', estimatedMinutes: 4, totalFields: 19,
...
function appHeader() {
  ...
  return `<header class="product-header">
    ...
    <div class="product-header-tools">
      <label class="header-select"><span>Language</span><select data-language><option ${state.profile.language === 'English' ? 'selected' : ''}>English</option><option ${state.profile.language === 'हिंदी' ? 'selected' : ''}>हिंदी</option><option ${state.profile.language === 'தமிழ்' ? 'selected' : ''}>தமிழ்</option></select></label>
...
function documentsPage() {
  ...
  return privatePage(`<div class="content-panel documents-panel"><p class="eyebrow">DOCUMENT READINESS</p><h1>${formatDetails}</h1><p>Exact upload limits come from the ${service.service} service configuration.</p><div class="readiness-list"><div class="readiness-item"><div class="file-symbol">ID</div><div><strong>Identity document</strong><span>PDF · 640 KB · name and date of birth matched</span></div><em class="ready">✓ Ready</em></div><div class="readiness-item"><div class="file-symbol">AP</div><div><strong>Address proof</strong><span>PDF · 1.2 MB · selected current address matched</span></div><em class="ready">✓ Ready</em></div>${service.id === 'scholarship-application' ? `<div class="readiness-item"><div class="file-symbol">EDU</div><div><strong>Educational certificate</strong><span>PDF · 720 KB · qualification matched</span></div><em class="ready">✓ Ready</em></div>` : ''}<div class="readiness-item photo-readiness"><div class="file-symbol">IMG</div><div><strong>Photograph</strong><span>${state.photoFixed ? 'JPG · 413 × 531 px · 42 KB · metadata removed' : 'JPG · 920 × 1200 px · 310 KB'}</span></div>${state.photoFixed ? `<em class="ready">✓ Ready</em>` : `<em class="needs-fix">Needs correction</em>`}</div>${needsSignature ? `<div class="readiness-item photo-readiness"><div class="file-symbol">SIG</div><div><strong>Signature</strong><span>${state.signatureFixed ? 'JPG · 140 × 60 px · 18 KB · metadata removed' : 'JPG · 98 × 48 px · 74 KB'}</span></div>${state.signatureFixed ? `<em class="ready">✓ Ready</em>` : `<em class="needs-fix">Needs correction</em>`}</div>` : ''}</div>${state.photoFixed ? `<div class="photo-comparison"><div><span>Before</span><div class="photo-placeholder before-photo">310 KB</div><small>920 × 1200 px</small></div><div class="format-arrow"></div><div><span>After</span><div class="photo-placeholder after-photo">42 KB ✓</div><small>413 × 531 px</small></div><ul><li>Crop completed</li><li>Resized to configured dimensions</li><li>Compressed under 50 KB</li><li>Face visibility check simulated</li><li>Metadata removed</li></ul></div>` : `<div class="photo-rule"><strong>Photo required</strong><code>JPG/JPEG · 413 × 531 px · under 50 KB</code><button class="button button-primary" data-action="fix-photo">Fix photograph </button></div>`}${needsSignature ? (state.signatureFixed ? `<div class="signature-success">✓ Signature ready — JPG · 140 × 60 px · 18 KB</div>` : `<div class="photo-rule"><strong>Signature required</strong><code>JPG/JPEG · 140 × 60 px · under 30 KB</code><button class="button button-primary" data-action="fix-signature">Fix signature </button></div>`) : ''}<div class="route-actions"><button class="button button-outline" data-nav="/applications/new/questions">Back</button><button class="button button-primary" data-action="documents-next" ${ready ? '' : 'disabled'}>Review application </button></div></div>`, { step: 'STEP 6 OF 8', title: 'Document readiness' });
}
...
function trackingPage(applicationId) {
  ...
  return privatePage(`<div class="content-panel tracking-panel"><button class="back-row" data-nav="/dashboard">← Back to dashboard</button><p class="eyebrow">APPLICATION TRACKING</p><h1>${h(application.service)}</h1><p>${h(application.id)} · Last updated ${h(application.updatedAt || application.submittedAt)}</p><div class="tracking-timeline">${statuses.map((status, index) => `<div class="tracking-step ${index <= current ? 'complete' : ''} ${index === current ? 'current' : ''}"><i>${index < current ? '✓' : index + 1}</i><strong>${status}</strong><span>${index <= current ? (index === current ? 'Current status' : 'Completed') : 'Pending'}</span></div>`).join('')}</div><div class="simulated-callout">${mockBadge('MOCK STATUS CONTROL')}<p>For the prototype, advance the application to demonstrate tracking.</p></div><div class="route-actions"><button class="button button-primary" data-action="advance-status" ${current === statuses.length - 1 ? 'disabled' : ''}>Simulate next status </button></div></div>`, { step: 'TRACKING', title: application.id });
}

function privacyPage() {
  const savedApplications = applicationHistory();
  const content = `<div class="content-panel privacy-controls"><button class="back-row" data-nav="${state.authenticated ? '/dashboard' : '/'}">← Back</button><p class="eyebrow">PROFILE & PRIVACY CONTROLS</p><h1>See and control your information.</h1><p>Review what was connected, what each service received and when consent was provided.</p><div class="privacy-grid"><section><h2>Connected documents</h2><div class="privacy-list">${DOCUMENTS.slice(1).map((document) => `<div><span>${document.label}</span><strong>${state.documentsConnected ? 'Connected' : 'Not connected'}</strong></div>`).join('')}</div></section><section><h2>Saved information</h2><div class="privacy-list"><div><span>Basic profile</span><strong>${h(state.profile.fullName)}</strong></div><div><span>Preferred language</span><strong>${h(state.profile.language)}</strong></div><div><span>Temporary draft</span><strong>${state.answers.occupation ? 'Stored in this session' : 'None'}</strong></div></div></section><section class="full-privacy-section"><h2>Consent history</h2>${savedApplications.length ? `<div class="consent-history"><div class="consent-history-head"><span>Service</span><span>Information shared</span><span>Provided</span></div>${savedApplications.map((item) => `<div><strong>${h(item.service)}</strong><span>18 fields · 3 documents</span><span>${h(item.submittedAt)}</span></div>`).join('')}</div>` : `<div class="empty-state">No consent receipts yet.</div>`}</section></div><div class="danger-zone"><div><strong>Delete saved demo profile</strong><span>Clears temporary profile and document connection from this browser.</span></div><button class="button button-outline" data-action="delete-profile">Delete saved profile</button></div>${state.authenticated ? `<button class="button button-primary" data-action="signout">Sign out securely</button>` : ''}</div>`;
...
document.addEventListener('click', (event) => {
  ...
  if (action === 'voice-input') {
    ...
    event.target.closest('button').textContent = '✓ Voice demo captured';
  }
